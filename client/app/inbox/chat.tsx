...
export default function Page({ route }) {
  ...
  const sendMessage = async () => {
    ...
    let { error } = await supabase.from("chatroom").upsert([
      {
        user_id: await GetUserId(),
        other_user_id: route.params.id,
        last_message: message,
        last_message_sent_by: await GetUserId(),
      },
      {
        user_id: route.params.id,
        other_user_id: await GetUserId(),
        last_message: message,
        last_message_sent_by: await GetUserId(),
      },
    ]);
